...
def scale_input(x_in, x_mean, x_std):
    return (x_in - x_mean)/x_std
...
def j_wb(x,y,w,b,Lambda = 0,eplison =  1e-15):
    m = y.shape[0]
    f = f_wb(z(x,w,b))
    cost = (-(np.sum(y*np.log(f+eplison)+ (1-y)* np.log(1-f+eplison)))/m + ((Lambda)/(2*m)) * np.sum(w**2)) 
    return cost
...
def compute_gradients(x,y,w,b,Lambda = 0):
    ...
    return dj_dw, dj_db 
def alpha_backtracking(x,y,w,b, alpha,dj_dw, dj_db, Lambda = 0):
    
    curr_cost = j_wb(x,y,w,b,Lambda)
    w_new =  w - alpha*dj_dw 
    b_new =  b - alpha*dj_db
    new_cost = j_wb(x,y,w_new,b_new,Lambda)
    while new_cost > curr_cost and alpha > 1e-8:
        alpha*= 0.97
        w_new =  w - alpha*dj_dw 
        b_new =  b - alpha*dj_db
        new_cost = j_wb(x,y,w_new,b_new, Lambda)
...
"""
    ...
    for  i in range(num_iter):
        ...
        alpha = alpha_backtracking(x,y,w_tmp,b,alpha,dj_dw, dj_db,Lambda)
        ...
        # print('abs(curr_cost - prev_cost):',abs(curr_cost - prev_cost))
